...
| 清单条目 | 内容 | 本单元对应 |
|---------|------|----------|
| Item 5-7 | 纳排标准、信息来源与完整检索式 | TODO1: arxiv 多查询检索 + protocol |
| Item 8 | 选择过程（筛选者与自动化工具） | TODO2-3: 去重 + 双人独立筛选 + kappa |
| Item 9-10 | 数据收集过程与数据项 | TODO4: 字段抽取 + 质量代理指标 |
| Item 11 | 研究内偏倚评估 | TODO4: RoB proxy + 正式人工评估计划 |
| Item 13-15 | 综合方法、报告偏倚与证据确定性 | TODO5 + protocol: ASReview proxy、偏倚与确定性计划 |
| Item 16a-17 | 流程图与研究数量 | TODO6: PRISMA flow diagram |
| Item 24 | 注册与协议 | `protocol.md` + amendment 日志 |

## 预注册、偏倚与证据确定性

本单元对应质量契约 **CQ-R4-1**。最低质量线是遵循 PRISMA 2020 声明（https://www.prisma-statement.org/prisma-2020）做可审计报告，而不是只生成流程图。正式交付前必须先冻结 `protocol.md`：研究问题、数据库范围、检索式、纳排标准、双人独立筛选规则、冲突裁决、证据综合方法、自动化工具披露与版本锁都要写入协议。任何改变检索式、纳排标准、种子集、RoB 阈值或 ASReview 停止规则的行为，都必须作为 **protocol amendment** 记录修改时间、修改原因和对 PRISMA 数字的影响。

- **双人独立筛选**：真实研究必须由两名筛选者分别记录 title/abstract 与 full-text 决策，并保存排除理由；本 notebook 里的 TODO3 目前是教学用模拟筛选（第二位筛选者由随机翻转生成），只能训练 Cohen's kappa 计算，不能冒充真实双人独立筛选证据。
- **研究内偏倚**：PRISMA 2020 Item 11 要求说明研究内偏倚评估方法。TODO4 的 Kitchenham 五维评分是 RoB proxy，用标题和摘要关键词近似判断研究质量；正式综述必须把每篇纳入文献的研究设计、数据来源、测量偏差、选择偏差、混杂控制和局限说明逐项记录。
- **报告偏倚**：PRISMA 2020 Item 14 要求说明如何评估因缺失结果导致的报告偏倚；正式报告还要披露数据库覆盖偏差、arXiv 学科偏差、只纳入英文/开放摘要以及未发表结果不可见的风险。
- **证据确定性**：纳入文献不能只给 Low/Moderate/High RoB，还要给证据确定性等级（High/Moderate/Low/Very low）及降级理由：研究内偏倚、不一致性、间接性、不精确性和发表/报告偏倚。
- **自动化工具披露**：PRISMA 2020 Item 8/9/13 分别要求交代选择、数据收集与综合过程，其中包括所用自动化工具；PRISMA 2020 Item 24 要求报告注册、协议位置及 amendment。凡使用 arxiv API、pandas、scikit-learn/ASReview、LLM 或 RAGAS，都要披露工具名、版本、参数、随机种子、人工复核比例和失败/回退规则。
- **数字漂移风险**：`research.md`、`data/README.md`、流程图和 notebook 输出必须来自同一 run manifest。当前冻结 fallback 元数据为 210→135→40→23；历史在线记录为 210→135→44→26，二者不得混写成同一次运行。
