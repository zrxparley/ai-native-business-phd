...
本单元新增 `from_scratch.md`，把 IMRaD 结构校验与引文图谱从 arxiv/statsmodels 库调用下沉到 AI工程从零构建的 numpy/re 数学推导层。scratch_topic：手写 IMRaD 结构完整性校验器（章节依赖 DAG 拓扑序 + 支撑完备性 + 引文接地）+ BibTeX 引文图谱解析，对应 rohitg00 P14 Agent Observability Platforms（结构化记录的可审计性），链接取自 `_from_scratch_map/module-r-research-methodology.md`。core_algorithm 把 IMRaD 建模为修辞性依赖 DAG $G=(V,E)$，推导三条完整性约束：(C1) 拓扑无环 $\sigma(u)<\sigma(v)$；(C2) 支撑完备性 $\forall v\in R:\exists(u,v)\in E,u\in M$ 且 $\forall v\in D:\exists u_1\in R,\exists u_2\in I$；(C3) 引文接地 $\text{indeg}_{cite}(v)\geq 1$，并定义引用密度 $\rho(s)=\tfrac{1}{|C_s|}\sum|\text{cites}|$，把"论文结构合理"从 LLM 主观评分转成可计算图论约束。code_artifact 手写 numpy/re（≤50 行，imports 严格白名单），实现 `topo_valid` + `integrity` + `bibtex_parse` + `citation_density`，verification_property 锚定"合法 IMRaD integrity 返回空、残缺返回 $\geq 3$ 条 + 拓扑序 I,M,R,D 合法而 I,R,M,D 非法"。connection_to_unit 给出 ≥3 delta：TODO1 关键词分类只判单句归属 vs from scratch DAG 校验跨节依赖；TODO6 LLM-as-judge 主观 vs integrity 客观前置；BibTeX 半结构化文本解析暴露引文原始形态。exercises 绑定 `starter.ipynb` TODO1（句级分类）与 `practice.md` IMRaD drill。rohitg00 深链见 from_scratch.md deep_dive_links 节（ai-engineering-from-scratch 仓库 P14/24）。本层与 v5.0 arxiv/statsmodels/LLM-judge 实现互补：库层教"如何执行 IMRaD 写作流程"，from scratch 层教"如何用数学推导审计论文结构自洽"--前者是工程底座，后者是方法论底座。
