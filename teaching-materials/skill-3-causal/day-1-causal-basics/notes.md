...
3. 能用 NSW 随机实验建立基准，并在 NSW 处理组 + CPS 观测对照中比较朴素均值差、后门调整与实验基准，解释选择偏差和识别边界
...
> CQ-S3-1 质量契约：本单元的目标不是让学生“跑出一个 ATE 数字”，而是让学生能写出可审计的因果主张、识别假设、诊断证据与反驳边界。

### 1. estimand：先定义要估什么

本 Day 默认 estimand 是平均处理效应：

$$ATE = E[Y(1) - Y(0)]$$

在 NSW/CPS 观测比较中，`T=treat`，`Y=re78`，候选调整变量 `X={age, educ, black, hisp, marr, nodegree, re74, re75}`。在营销映射中，`T=收到优惠券/看到广告`，`Y=转化率/GMV/客单价`，`X=用户画像、历史消费、历史活跃度、设备、渠道、地区等处理前变量`。

### 2. 四个必须明说的识别假设

1. **一致性（consistency）**：同一个处理状态含义必须清楚。`treat=1` 不能同时表示“培训强度不同的一组项目”；营销中“收到优惠券”也必须定义渠道、时间窗、折扣强度。
2. **可交换性（exchangeability）/ 无未观测混杂**：在控制 `X` 后，处理分配与潜在结果独立。若存在未记录的就业动机、销售主动筛选、高价值用户标签等变量同时影响 `T` 与 `Y`，后门调整仍会有偏。
3. **正值性（positivity）/ overlap**：每类可比人群都必须同时有处理组和对照组。若某些高价值用户几乎全被投放，或某些低价值用户从不被投放，就不能从数据中学习这些人群的反事实。
4. **SUTVA / 无干扰**：一个用户是否被处理不应改变另一个用户的结果。营销中若优惠券通过社交传播、库存约束或竞价机制影响未处理用户，需在讨论中说明风险。

### 3. overlap 与估计量诊断

`starter.ipynb` 的 TODO2 不应只看处理/对照均值。高质量提交需要补三类诊断：

- **协变量平衡**：报告每个混杂变量的标准化均值差（standardized mean difference, SMD），而不只是原始均值差；SMD < 0.1 通常可作为平衡较好的经验线，0.1–0.25 需要解释，>0.25 需要修正模型或缩小可解释人群。
- **positivity/overlap**：估计倾向得分 `P(T=1|X)` 后画处理组/对照组重叠区，报告共同支撑区之外的样本占比。
- **估计量透明度**：说明 `backdoor.linear_regression`、PSM 或其他估计器各自依赖的函数形式假设；不要把 DoWhy 输出当成自动真值。

### 4. 负对照、refuter 与敏感性分析

因果结论必须经受“它可能错在哪里”的检查。最低要求：

1. **负对照**：设计一个理论上不应被处理影响的结果变量，或一个理论上不应影响结果的处理变量。若负对照也显示显著效应，说明仍可能有残余混杂或模型错设。
2. **placebo_treatment_refuter**：随机置换处理变量后，新估计应接近 0；否则说明估计器可能在噪声里“发现”效应。
3. **random_common_cause_refuter**：加入随机协变量后，核心估计不应发生实质变化；否则说明估计对无关变量过敏。
4. **data_subset_refuter / bootstrap_refuter**：在子样本或 bootstrap 下重复估计，检查 ATE 方向、量级和置信区间是否稳定。
5. **未观测混杂敏感性分析**：写出一个最可能遗漏的混杂变量，并回答：“它需要同时多强地影响 `T` 和 `Y`，才会推翻当前结论？”

DoWhy refuter 官方说明：https://www.pywhy.org/dowhy/v0.14/user_guide/refuting_causal_estimates/
...
v5.0 改用 Lalonde 教学数据，但必须区分来源：`nsw_mixtape` 是 445 行随机实验样本，`cps_mixtape` 是观测对照库。本上机以 NSW 实验组内均值差作基准，再把 NSW 处理组与 CPS 对照组拼成观测比较，显式构造文献中的选择偏差问题。**实验样本本身不应被描述成“严重混杂”**。
...
| `age`, `educ`, `black`, `hisp`, `marr`, `nodegree`, `re74`, `re75` | 用户画像特征（活跃度/历史消费/设备…） | 处理前协变量 X |

**你要回答的因果问题**：NSW 实验基准是多少？NSW 处理组与 CPS 观测对照的朴素差为何偏离基准？后门调整在什么 overlap 与 exchangeability 假设下可能缩小差距？

### 上机任务（6 个 TODO，见 starter.ipynb）

1. **TODO1**：分别加载 `causaldata.nsw_mixtape` 与 `causaldata.cps_mixtape`，构造实验基准样本和观测比较样本
2. **TODO2**：用 SMD 与倾向得分 overlap 探索观测样本的可比性
3. **TODO3**：分别计算 NSW 实验基准与观测样本朴素均值差
...
作业、5 分制量表、费曼演练、2 分钟话术、复盘自诊表——沿用独立教材 § Day 1 既有设计。本学习材料包不重复理论型任务，仅新增上机交付物；是否达标以 `practice.md` / `alignment.md` 的 CQ-S3-1 rubric 为准：

**新增交付物（v5.0）**：
- [ ] 完成的 `starter.ipynb`（6 个 TODO 全部填好，能跑通）
- [ ] 一段 300 字分析：实验基准、观测朴素估计与后门调整估计为何不同？哪些差异来自样本选择，哪些假设无法由数据验证？
...
> v11.0 新增 [`from_scratch.md`](./from_scratch.md)：AI工程从零构建，与本单元 DoWhy 库实现形成"from-scratch vs 库"对照。
> - **从零构建主题**：手写后门调整估计器（backdoor adjustment from scratch）
> - **核心算法**：$P(y|do(t))=\sum_z P(y|t,z)P(z)$（含数学推导 + LaTeX，do-演算后门准则）
> - **code_artifact**：手写 numpy 骨架（≤50行），imports ⊆ {numpy}，附 verification_property（ATE 收敛真值）
> - **延伸阅读**：rohitg00 AI工程 from scratch P2/09 Model Evaluation + P1/21 Graph Theory
> - **手写实现要点**：用 from-scratch numpy 分层加权而非 DoWhy 黑箱，理解"控制混杂"到金属层
> - **verification_property**：后门调整 ATE 收敛到构造真值 2.0；朴素估计在有混杂时有偏
